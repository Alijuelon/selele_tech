...
const team = ref([
  {
    role: 'Windows & Hardware Specialist', 
    name: 'Reza Saputro Dwi Prastyo Widodo',
    shortBio: 'Mahasiswa pecinta hardware. Spesialis rakit, upgrade, dan optimalisasi PC/Laptop.',
    photo: '/reza.jpeg',
    fullBio: 'Berawal dari hobi sejak bangku SMP dan SMA, Reza sangat menyukai ekosistem Windows dan dunia hardware. Ia berpengalaman dalam instal ulang OS, merakit PC, upgrade komponen, hingga pembersihan rutin (cleaning) agar performa PC dan laptop selalu optimal dan kencang.',
    socials: { linkedin: '#', github: '#', instagram: '#' },
    skills: ['PC Building', 'Windows Troubleshooting', 'Hardware Upgrade', 'PC Cleaning & Maintenance'],
    projects: [
      { 
        title: 'Optimalisasi Laptop Lawas', 
        desc: 'Melakukan instal ulang Windows, pembersihan debu (re-paste thermal), dan upgrade SSD/RAM.' 
      },
      { 
        title: 'Custom PC Build', 
        desc: 'Merakit PC untuk gaming dan produktivitas dengan manajemen kabel yang rapi.' 
      }
    ]
  },
  {
    role: 'Software Architecture & IoT', name: 'Ali juelon Sinaga',
  ...
  {
    role: 'Network Security Architect', name: 'M. Ridho Nugraha',
  ...
  <section id="operatives" class="py-24 relative z-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
        <div v-for="(person, index) in team" :key="index" @click="openModal(person)"
             class="card-3d-wrap animate-fadeInUp opacity-0 cursor-pointer" :class="`delay-${(index+1)*100}`" style="animation-fill-mode: forwards;">
          <div class="card-3d glass-panel p-8 rounded-2xl h-full flex flex-col group border border-slate-200 dark:border-slate-800 transition-all hover:border-cyan-500/30 overflow-hidden relative">
            ...
            <div class="w-24 h-24 rounded-full overflow-hidden mb-8 border-2 border-cyan-500/50 group-hover:border-cyan-400 group-hover:glow-pulse transition-all duration-300 relative z-10">
              <video v-if="person.video" :src="person.video" autoplay loop muted playsinline class="object-cover w-full h-full grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700 pointer-events-none"></video>
              <img v-else :src="person.photo" alt="Photo" class="object-cover w-full h-full grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700 pointer-events-none" />
            </div>
            ...
            <div class="relative z-10 flex flex-col h-full">
                ...
                <p class="text-slate-600 dark:text-slate-400 text-sm mb-6 leading-relaxed text-justify">{{ person.shortBio }}</p>
    ...
    <Teleport to="body">
      <Transition name="backdrop">
          <div v-if="activeMember" class="fixed inset-0 z-[99998] bg-slate-950/90 backdrop-blur-md cursor-pointer" @click="closeModal"></div>
      </Transition>

      <Transition name="flip">
          <div v-if="activeMember" class="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 lg:p-10 cursor-auto overscroll-contain" @click="closeModal">
              
              <div class="relative w-full max-w-6xl max-h-[90dvh] bg-slate-100 dark:bg-slate-950 rounded-2xl shadow-2xl overflow-y-auto border border-slate-200 dark:border-cyan-900/50 custom-scrollbar flex flex-col md:flex-row shadow-cyan-500/5 cursor-auto" @click.stop>
                  
                  <button @click="closeModal" class="absolute top-5 right-5 z-50 w-12 h-12 rounded-full glass-panel bg-slate-900/80 border border-slate-700 text-white flex items-center justify-center hover:bg-cyan-500 hover:scale-110 transition-all text-2xl font-black cursor-pointer">✕</button>
                  ...
                  <div class="md:w-[35%] bg-slate-200 dark:bg-slate-900 md:sticky top-0 h-auto md:h-full border-b md:border-b-0 md:border-r border-slate-300 dark:border-slate-800">
                    <div class="h-80 md:h-[65%] w-full relative">
                      ...
                      <video v-if="activeMember.video" :src="activeMember.video" autoplay loop muted playsinline class="object-cover w-full h-full opacity-90 dark:opacity-70 grayscale hover:grayscale-0 transition-opacity"></video>
                      <img v-else :src="activeMember.photo" class="object-cover w-full h-full opacity-90 dark:opacity-70 grayscale hover:grayscale-0 transition-opacity" />
                    </div>
                  ...
                  <div class="p-8 md:p-14 md:w-[65%] text-slate-800 dark:text-slate-300">
                    ...
                    <p class="text-base leading-relaxed mb-12 text-slate-700 dark:text-slate-400 font-medium text-justify">{{ activeMember.fullBio }}</p>
                    ...
                    <div class="pb-10">
                      ...
                      <div class="space-y-6">
                        <div v-for="proj in activeMember.projects" :key="proj.title" class="p-6 bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl border-l-4 border-l-cyan-500 hover:border-slate-300 dark:hover:border-slate-700 transition-colors shadow-sm">
                          ...
                          <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed text-justify">{{ proj.desc }}</p>
                        </div>
                      </div>
                    </div>
                  </div>
